...
export function setAppConfig(data?: any) {
...
export function getConfDir(configFile?:string) {
	let appConfig = getAppConfig();
 ...
 */
export function getConfig(cfname: string) {
...
export function setConfig(cfname: string, data:GenericObject) {
	if (path.extname(cfname) !== '.json5') {
		cfname += '.json5';
	}
	let cdir = getConfDir();
...
export function setConfDir(confPath: string) {
